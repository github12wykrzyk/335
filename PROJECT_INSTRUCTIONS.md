...
10. DOSTARCZENIE FUNKCJI DO GRY (NIE KOŃCZ NA ADAPTERZE)
Na polecenie „dokończ”, „buduj do gry” i „zintegruj z loaderem” wynik izolowanej kompilacji x86 nie jest zakończeniem zadania. AI ma implementować brakujące polityki klienta, wspólną obsługę loadera/updatera, zarejestrować aktywne DLL z dokładnymi bajtami, uruchomić właściwe buildy i bramki paczki. Osobny workflow autopickpocket_delivery.yml raportuje jawnie, które warunki integracji nie zostały spełnione; jego brak PASS blokuje stwierdzenie „gotowe do instalacji”. Po FINAL_PACKAGE: PASS nadal wymagany jest test w grze na tym samym SHA; AI nie przedstawia nieprzeprowadzonego testu jako PASS. Brak dowodu w grze nie zwalnia z implementacji; gdy istnieje nieusuwalny w bieżącym środowisku blokujący fakt, podaj go i nie wydawaj pozornej paczki.
