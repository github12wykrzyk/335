...
## 2026-09-24: przyczyna z danych native_w2s w grze

Otrzymany log PlayerESP(1).jsonl zawiera 2 probki `native_w2s`. Dla rozdzielczosci 2560x1440 klient podaje `render_lbrt=[0,0,1,1]`, ale `clip_lbrt=[0,0,0.4903,0.8716]`. W poprzednim kodzie surowe `raw_xy=[0.0908,0.1965]` zamieniano blednie na `[0.1852,0.2254]` przez dzielenie przez CLIP, czyli x ponad 2x, oraz y przez 0.8716. Z badanego kodu WoW 12340 funkcja W2S juz uwzglednia render extents +0x330..0x33c przy wyliczaniu screen[0..1]. Obecnie uzywamy tej domeny przy jednokrotnym przejsciu na UI (top-down); Lua kotwiczy etykiete do WorldFrame (bottom-left). Nowy test C sprawdza obie probki z logu. In-game pixel-perfect nie jest potwierdzone; raporty nie zawieraja GUID ani pozycji postaci. Test nowego SHA jest wymagany. Nie przywracac dzielenia przez +0x64..0x70 ani nie mnozyc FoV.
