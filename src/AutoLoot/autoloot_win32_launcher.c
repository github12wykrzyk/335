/* Authorized stock 12340 x86 launcher, not an installer or a replacement for
 * the verified github12wykrzyk/335 updater. No anti-cheat bypass or EXE patch.
 * Explicit local CLI target only; the x86 updater must eventually own launch,
 * signed manifests, rollback and complete-stack installation.
 */
...
int wmain(int argc, wchar_t **argv) {
    ...
    *wcsrchr(command, L'\\') = L'\0';
    wcscat_s(command, 2*MAX_PATH, L"\\AutoLoot335_Host_UNREGISTERED.dll");
