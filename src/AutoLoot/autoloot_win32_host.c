/* Isolated native frame experiment for exact WoW 3.3.5a build 12340 x86.
 * The authorized loader uses window hooks ONLY for a one-time game-thread
 * initialization/arm request. Loot ticks run from CGWorldFrame::OnLayerUpdate,
 * never from mouse/keyboard/window-message traffic or a foreign worker thread.
 * Original frame dispatch is tail-called without altering arguments or ABI.
 */
...
#define AL_WORLD_FRAME_GLOBAL ((uintptr_t)0x00EEEA8Cu)
#define AL_WORLD_LAYER_SLOT ((uintptr_t)0x009F9A50u)
#define AL_WORLD_LAYER_ORIGINAL ((uintptr_t)0x004FA040u)
...
static void *volatile g_original_frame;
static void *volatile g_frame_slot;
static volatile LONG g_frame_ready;
static volatile LONG g_frame_wrong_thread;
static volatile LONG g_frame_pulses;
static volatile LONG g_frame_ticks;
...
/* The only use of window hooks is to bind and arm the vtable once.
 * They never run the loot engine; held input cannot starve an OnLayerUpdate.
 * Static SHA-pinned audit: exact OnLayerUpdate 0x004FA040 (55 8B EC 83
 * EC 3C D9 45 08), unique read-only vtable reference 0x009F9A50.
 * World-frame global 0x00EEEA8C is checked in process after it exists.
 * These observations are necessary but not runtime proof of thread/behavior.
 */
__declspec(dllexport) UINT WINAPI AL335_MessageId(void) {
    return RegisterWindowMessageA(AL_WINMSG_NAME);
}

static __declspec(noinline) void AL335_FrameTick(void) {
    DWORD now;
    if (!g_engine.bound || !g_engine.engine.enabled) return;
    if (!is_game_thread()) {
        InterlockedIncrement(&g_frame_wrong_thread);
        return;
    }
    InterlockedIncrement(&g_frame_pulses);
    if (g_driving) return;
    now=GetTickCount();
    ...
    InterlockedIncrement(&g_frame_ticks);
    g_driving=0u;
}

/* Preserve registers, EFLAGS, arguments and caller's stack; original x86
 * thiscall receives the unchanged ECX and the original return address.
 * The compiler handles FP cleanup inside AL335_FrameTick. There is no
 * guessed float/return-value ABI in the trampoline.
 */
__declspec(naked) static void AL335_FrameThunk(void) {
    __asm {
        pushfd
        pushad
        call AL335_FrameTick
        popad
        popfd
        jmp dword ptr [g_original_frame]
    }
}
static int AL335_ArmFrame(void) {
    uint32_t world=0u, vtable=0u, original=0u;
    uintptr_t slot=AL_WORLD_LAYER_SLOT;
    DWORD previous=0, restored=0;
    PVOID observed;
    if (g_frame_ready) return 1;
    if (!g_engine.bound || !is_game_thread() || !g_engine.engine.enabled)
        return 0;
    if (!read32(NULL,AL_WORLD_FRAME_GLOBAL,&world) ||
        world<AL_FIRST_VALID_PTR || world>=AL_MAX_VALID_PTR ||
        !read32(NULL,(uintptr_t)world,&vtable) ||
        vtable<AL_FIRST_VALID_PTR || vtable>=AL_MAX_VALID_PTR ||
        (vtable & 3u) || vtable>slot || slot-vtable>=512u ||
        !read32(NULL,slot,&original) ||
        original!=(uint32_t)AL_WORLD_LAYER_ORIGINAL)
        return 0; /* World frame not yet created or unrecognized vtable */
    /* Verify exact target instructions in the SHA-pinned executable. */
    {
        static const BYTE expected[]={
            0x55,0x8b,0xec,0x83,0xec,0x3c,0xd9,0x45,0x08
        };
        if (!equal_bytes(AL_WORLD_LAYER_ORIGINAL,expected,sizeof(expected)))
            return 0;
    }
    if (!VirtualProtect((void *)slot,sizeof(void *),PAGE_READWRITE,&previous))
        return 0;
    g_original_frame=(void *)AL_WORLD_LAYER_ORIGINAL;
    observed=InterlockedCompareExchangePointer((PVOID volatile *)slot,
                  (PVOID)AL335_FrameThunk,(PVOID)AL_WORLD_LAYER_ORIGINAL);
    if (observed==(PVOID)AL_WORLD_LAYER_ORIGINAL) {
        g_frame_slot=(void *)slot;
        InterlockedExchange(&g_frame_ready,1);
    } else {
        g_original_frame=NULL;
    }
    VirtualProtect((void *)slot,sizeof(void *),previous,&restored);
    return g_frame_ready!=0;
}
__declspec(dllexport) LONG WINAPI AL335_FrameStatus(void) {
    /* 0 not armed, 1 active, 2 foreign-thread frame callback observed. */
    if (g_frame_wrong_thread) return 2;
    return g_frame_ready ? 1 : 0;
}
__declspec(dllexport) LONG WINAPI AL335_FrameTicks(void) {
    return g_frame_ticks;
}
static void handle_control(UINT message, WPARAM command) {
    ...
    if (!g_init_attempted) {
        ...
        if (!bind_host()) { g_game_thread=0u; return; }
    }
    ...
    if (command==1u || command==2u) AL335_ArmFrame();
}
__declspec(dllexport) LRESULT CALLBACK AL335_HookProc(int code, WPARAM wp, LPARAM lp) {
    ...
    msg=(MSG *)lp;
    if (msg->message!=WM_QUIT) handle_control(msg->message,msg->wParam);
