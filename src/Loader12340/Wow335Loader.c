/* WoW 3.3.5a 12340 x86 shared AutoLoot/AutoPickPocket loader source.
 * This DLL is imported by a copy-only patched EpochConnection.dll at process startup.
 * This source is NOT an installable package until exact binary/manifest gates pass.
 */
...
#include <wincrypt.h>
#include <string.h>
#include "../AutoPickPocket/autopickpocket_win32_host.h"
...
/* Load both modules before installing a SINGLE shared pair of AutoLoot-owned
 * window-thread hooks. PP callbacks are driven by AutoLoot's hook, never from
 * the worker and never from a second independent hook.
 *
 * PP policy must be provided by independently audited game-policy code. If
 * the provider or any required function is absent, reject the two-DLL stack
 * BEFORE activating either module; do not synthesize success or cast blindly.
 */
typedef const Pp335Policy *(__stdcall *pp_policy_fn)(void);
typedef int (__stdcall *pp_bind_fn)(const void *);
typedef void (__stdcall *pp_tick_fn)(uint32_t);
typedef void (__stdcall *pp_enable_fn)(int);
typedef int (__stdcall *al_peer_config_fn)(const void *,pp_bind_fn,pp_tick_fn,pp_enable_fn);
typedef LONG (__stdcall *al_peer_status_fn)(void);
static FARPROC export_stdcall(HMODULE module,const char *plain,const char *decorated) {
    FARPROC fn=GetProcAddress(module,plain);
    return fn ? fn : GetProcAddress(module,decorated);
}
static int configure_pp_peer(HMODULE autoloot, HMODULE pp,
                             al_peer_status_fn *peer_status) {
    pp_policy_fn provider=(pp_policy_fn)export_stdcall(
        pp,"PP335_VerifiedPolicyV1","_PP335_VerifiedPolicyV1@0");
    pp_bind_fn bind=(pp_bind_fn)export_stdcall(
        pp,"PP335_BindOnGameThread","_PP335_BindOnGameThread@4");
    pp_tick_fn tick=(pp_tick_fn)export_stdcall(
        pp,"PP335_TickOnGameThread","_PP335_TickOnGameThread@4");
    pp_enable_fn enable=(pp_enable_fn)export_stdcall(
        pp,"PP335_EnableOnGameThread","_PP335_EnableOnGameThread@4");
    al_peer_config_fn configure=(al_peer_config_fn)export_stdcall(
        autoloot,"AL335_ConfigurePeer","_AL335_ConfigurePeer@16");
    const Pp335Policy *policy;
    *peer_status=(al_peer_status_fn)export_stdcall(
        autoloot,"AL335_PeerStatus","_AL335_PeerStatus@0");
    if (!provider || !bind || !tick || !enable || !configure || !*peer_status) {
        log_event(L"AutoPickPocket335.dll",L"SHARED_EXPORTS_MISSING",ERROR_PROC_NOT_FOUND);
        return 0;
    }
    policy=provider(); /* provider owns the immutable policy for DLL lifetime */
    if (!policy || !policy->eligible_npc || !policy->spell_usable ||
        !policy->begin_attempt || !policy->cast_result ||
        !policy->world_token) {
        log_event(L"AutoPickPocket335.dll",L"POLICY_NOT_VERIFIED",ERROR_INVALID_DATA);
        return 0;
    }
    if (configure(policy,bind,tick,enable)!=1) {
        log_event(L"AutoPickPocket335.dll",L"SHARED_PEER_CONFIG_FAILED",ERROR_INVALID_STATE);
        return 0;
    }
    return 1;
}
static DWORD activate_autoloot(HMODULE host, HMODULE pp) {
    GameWindow game={0};
    ...
    UINT message;
    al_peer_status_fn peer_status=NULL;
    ...
    if (pp && !configure_pp_peer(host,pp,&peer_status)) return 1;
    ...
    if (pp && (!peer_status || peer_status()!=2)) {
        log_event(L"AutoPickPocket335.dll",L"GAME_THREAD_BIND_FAILED",ERROR_INVALID_STATE);
        goto cleanup;
    }
    log_event(L"AutoLoot335.dll",L"HOOK_ENABLED_RESPONSIVE",0);
    if (pp) log_event(L"AutoPickPocket335.dll",L"GAME_THREAD_BOUND_DISABLED",0);
    {
...
static DWORD WINAPI load_modules(LPVOID unused) {
    ...
    HMODULE autoloot=NULL, autopickpocket=NULL;
    FILE *f;
    (void)unused;
    ...
    for (unsigned int i = 0; i < count; ++i) {
        ...
        if (_wcsicmp(names[i], L"AutoPickPocket335.dll") == 0) autopickpocket=loaded;
    }
    if (autopickpocket && !autoloot) {
        log_event(L"AutoPickPocket335.dll",L"AUTOLOOT_SHARED_HOOK_REQUIRED",ERROR_INVALID_DATA);
        return 1;
    }
    if (autoloot) return activate_autoloot(autoloot,autopickpocket);
    return 0;
}
