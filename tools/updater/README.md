...
## 0.3.17-335-epoch-test — AutoLoot Native Frame (isolated TEST)
New native host ticks ONLY from the exact 12340 CGWorldFrame::OnLayerUpdate callback after checked vtable pointer/CPU prolog/executable SHA256 and correct game-window thread. WH_GETMESSAGE/WH_CALLWNDPROC provide initialization and arming only; their rate does not determine loot cadence after hook activation. The loader logs FRAME_ARMED and FRAME_TICKS_DELTA and refuses the old DLL without frame exports. Self-update installs the new updater first; then the existing Aktualizuj flow migrates the already managed old AutoLoot with backup/rollback, updates both manifests with truthful feature-source provenance, and installs the matching Epoch loader pair. New AutoLoot binary exact SHA256 e2bf85df42ca8c434db5a99a7606a7d3cf82995ea525fa8acec2c8cc7587579a. The isolated DLL and pair are PE32 x86 compiled but FINAL_PACKAGE and gameplay still unverified. STABLE/main and work remain unmodified.
