...
$stream=$assembly.GetManifestResourceStream("WoW335Runtime.Loader.exe")
if ($null -eq $stream) { throw "Missing universal runtime loader resource" }
...
$method=$form.GetMethod('TryLaunchInstalledModules',
...
if (-not $code.Contains('TryLaunchInstalledModules(root, exe, state)') -or
...
Write-Host "UNIVERSAL_RUNTIME_LOADER: PASS, embedded PE32 x86, manifest-gated launch"
