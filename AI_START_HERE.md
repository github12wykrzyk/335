...
3. main contains STABLE infrastructure only, with EMPTY active runtime. work and feature/* hold separate TEST experiments; main has no game package without accepted exact-SHA promotion.
4. Run python tools/verify_repo.py, python tools/verify_current.py and python -m unittest discover -s tests -v.
5. Exact user-selected 12340 x86 Wow.exe is pinned in runtime/client_exe_target.json. A runnable STABLE candidate requires user-accepted exact DLLs, SHA-addressed recoverable binaries and FINAL_PACKAGE: PASS. EMPTY main means infrastructure only.
